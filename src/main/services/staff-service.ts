...
import { ValidationUtils, ColorUtils } from '../../shared/utils';
import { ERROR_MESSAGES, EVENT_COLORS } from '../../shared/constants';
...
export class StaffService extends BaseService<Staff> {
     ...
     * スタッフ情報を更新
     */
    async update(id: number, data: Partial<StaffFormData>): Promise<ApiResponse<Staff>> {
        try {
            // 更新前のバリデーション
     ...
     * 作成前のバリデーション
     */
    protected async validateBeforeCreate(data: StaffFormData): Promise<void> {
        const errors: string[] = [];

        // 必須項目チェック
        const nameValidation = ValidationUtils.validateRequired(data.name);
        ...
        // 表示色のバリデーション
        if (data.displayColor) {
            if (!ColorUtils.isValidHexColor(data.displayColor)) {
     ...
     * 更新前のバリデーション
     */
    protected async validateBeforeUpdate(id: number, data: Partial<StaffFormData>): Promise<void> {
        const errors: string[] = [];
