// src/main/database/repositories/schedule-repository.ts
import { Database } from 'sqlite3';
import { BaseRepository } from './base-repository';
import { Schedule, ScheduleSearchParams, ApiResponse } from '../../../shared/types';
import { TABLE_NAMES } from '../../../shared/constants';

export class ScheduleRepository extends BaseRepository<Schedule> {
    constructor(db: Database) {
        super(db, TABLE_NAMES.SCHEDULES);
    }
    ...
    async searchSchedules(params: ScheduleSearchParams = {}): Promise<ApiResponse<Schedule[]>> {
        try {
            ...
            const schedules = await this.executeQuery<Schedule>(query, queryParams);
    ...
    async getSchedulesByDateRange(startDate: string, endDate: string): Promise<ApiResponse<Schedule[]>> {
        try {
            ...
            const schedules = await this.executeQuery<Schedule>(query, [startDate, endDate]);
            return { success: true, data: schedules };
     ...
     * スタッフの予定数をカウント
     */
    async getScheduleCountByStaff(staffId: number): Promise<number> {
        try {
            const result = await this.executeQuerySingle<{ count: number }>(
                `SELECT COUNT(*) as count FROM ${this.tableName} WHERE staff_id = ?`,
                [staffId]
            );
    ...
    async checkTimeConflict(
        ...
        try {
            ...
            const conflictingSchedules = await this.executeQuery<Schedule>(query, params);
    ...
    async getMonthlyStatistics(year: number, month: number): Promise<ApiResponse<{
        ...
        try {
            ...
            const completedResult = await this.executeQuerySingle<{ count: number }>(
                `SELECT COUNT(*) as count FROM ${this.tableName} 
                 WHERE start_datetime >= ? AND start_datetime < ? 
            ...
            // 請求状況別統計
            const billingResults = await this.executeQuery<{ billing_status: string; count: number }>(
                `SELECT billing_status, COUNT(*) as count FROM ${this.tableName} 
                 WHERE start_datetime >= ? AND start_datetime < ? 
            ...
            // 業務カテゴリ別統計
            const categoryResults = await this.executeQuery<{ business_category: string; count: number }>(
                `SELECT business_category, COUNT(*) as count FROM ${this.tableName} 
                 WHERE start_datetime >= ? AND start_datetime < ? 
                 GROUP BY business_category`,
                [startDate, endDate]
            );

            const billingStatistics: Record<string, number> = {};
            billingResults.forEach(result => {
                billingStatistics[result.billing_status] = result.count;
            });

            const categoryStatistics: Record<string, number> = {};
    ...
    async getTodaySchedules(): Promise<ApiResponse<Schedule[]>> {
        try {
            ...
            const schedules = await this.executeQuery<Schedule>(query, [today]);
            return { success: true, data: schedules };
    ...
    async getUnsyncedSchedules(): Promise<ApiResponse<Schedule[]>> {
        try {
            ...
            const schedules = await this.executeQuery<Schedule>(query, []);
            return { success: true, data: schedules };
    ...
    async updateGoogleEventId(scheduleId: number, googleEventId: string): Promise<ApiResponse<void>> {
        try {
            await this.executeUpdate(
                `UPDATE ${this.tableName} SET google_event_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
                [googleEventId, scheduleId]
            );
